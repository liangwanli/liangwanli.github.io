(() => {
  ...
  const copy = {
    zh: {
      ...
      publicationsTitle: "论文成果", accepted: "已录用", firstAuthor: "（第一作者）", paperDescription: "针对医学图像病灶尺度差异大、形态不规则以及编解码器语义差异问题，提出 MSM-Unet：使用多尺度 Mamba 模块联合建模局部细节和长程依赖，并通过增强型跳跃连接融合浅层边缘与深层语义特征。在 BraTS 数据集上，WT、TC、ET 分割指标均优于 TransUNet 和基准 Mamba 模型。", paperFigurePending: "论文配图待补充", paperLinkPending: "论文链接待公开后补充", projectFigurePending: "项目配图待补充",
